...
if (isset($images->image_fulltext) and !empty($images->image_fulltext)) {
	...
	$image = '<img'.$class.$title.' src="'.htmlspecialchars($images->image_fulltext).'" alt="'.htmlspecialchars($images->image_fulltext_alt).'" itemprop="image"/>';
}
...
<div id="system">
	...
	<article class="item"<?php if ($view != 'article') printf(' data-permalink="%s"', JRoute::_(ContentHelperRoute::getArticleRoute($this->item->slug, $this->item->catslug), true, -1)); ?> itemscope itemtype="http://schema.org/Article">
		...
		<header>
			...
			<h1 class="title" itemprop="name"><?php echo $this->escape($this->item->title); ?></h1>
				...
				<?php
					...
					if ($params->get('show_author') && !empty($this->item->author )) {
						...
						if (!empty($this->item->contactid) && $params->get('link_author') == true) {
							...
							echo JText::sprintf('COM_CONTENT_WRITTEN_BY', JHtml::_('link', JRoute::_($cntlink), $author, array('itemprop' => 'url')));
						} else {
					...
					if ($params->get('show_create_date')) {
						echo ' '.JText::_('TPL_WARP_ON').' <time datetime="'.substr($this->item->created, 0,10).'" itemprop="dateCreated">'.JHtml::_('date', $this->item->created, JText::_('DATE_FORMAT_LC3')).'</time>';
					}
		...
		<?php
			...
			if ($params->get('access-view')) {
				...
				echo '<div itemprop="articleBody">'.$this->item->text.'</div>';
